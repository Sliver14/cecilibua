...
export async function GET(req: Request) {
  try {
    ...
    // Send email if status is PAID and email hasn't been sent yet
    if (registration.status === 'PAID' && !registration.emailSent) {
      try {
        ...
        // We continue anyway, as we want to return the status
      }
    }
